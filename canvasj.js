...
window.addEventListener('resize', () => {
    if (typeof pattern !== 'undefined') generate(pattern);
});
...
function generate(json) {
    ...
    origCanvas.width = json.size_x;
    origCanvas.height = json.size_y;
    for (let y = 0; y < json.size_y; y++) {
        for (let x = 0; x < json.size_x; x++) {
            origCanvasCtx.fillStyle = json.colors[json.pattern[x + y * json.size_y]];
            origCanvasCtx.fillRect(x, y, 1, 1);
        }
    }
    ...
    if (json.behaviour == 'repeat') {
        ...
        while (bigHeight <= windowSizeY) {
            ...
            bigHeight += origCanvas.height - json.overlap.y;
        }
    }
}
